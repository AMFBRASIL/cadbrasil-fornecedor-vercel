...
export default function SicafAssistantChat() {
  ...
  const checkCnpjStatus = useCallback(async (cnpj: string) => {
    try {
      ...
      const res = await fetch(apiUrl(`/api/clients/by-documento/${encodeURIComponent(cnpjDigits)}`), {
        headers: getHeaders(),
      });
  ...
  const sendMessage = useCallback(async (text?: string) => {
    ...
    try {
      ...
      const res = await fetch(apiUrl("/api/sicaf-assistant/chat"), {
        method: "POST",
  ...
  const uploadFile = useCallback(async (file: File) => {
    ...
    try {
      ...
      const res = await fetch(apiUrl("/api/sicaf-assistant/upload"), {
        method: "POST",
